#!/bin/bash
# Build script for the textamp TUI binary (macOS and Linux).
# Usage: ./build-tui.sh [--makepackage | --clean | --help]
...
print_usage() {
    echo "Usage: ./build-tui.sh [OPTIONS]"
    echo ""
...
build_binary() {
    ...
    echo "Building $PKG_NAME $PKG_VERSION TUI (release)..."
...
class Textamp < Formula
  ...
  def install
    system "cargo", "build", "--release", "--no-default-features", "--features", "tui", "--bin", "textamp"
...
FORMULA
    ...
    echo ""
    echo "Install with:  brew install --formula dist/homebrew/${PKG_NAME}.rb"
}
