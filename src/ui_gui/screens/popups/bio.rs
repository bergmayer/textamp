...
pub fn view<'a>(p: &'a ArtistBioPopup) -> Element<'a, GuiMessage> {
    let body: Element<'a, GuiMessage> = if p.loading {
        text("Loading bio\u{2026}").size(13).into()
    } else if p.bio.is_empty() {
        text("No bio available.").size(13).into()
    } else {
        scrollable(text(&p.bio).size(13)).height(Length::Fixed(360.0)).into()
    };
