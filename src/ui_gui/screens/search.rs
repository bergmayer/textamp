...
pub fn view(state: &AppState) -> Element<'_, GuiMessage> {
    let query = text(format!("Query: {}", state.search.query)).size(12);

    let mut rows: Vec<Element<'_, GuiMessage>> = Vec::new();
    if let Some(results) = &state.search.results {
        if !results.artists.is_empty() {
            rows.push(text("\u{2014} Artists \u{2014}").size(12).into());
            for a in &results.artists {
                rows.push(text(format!("  {}", a.title)).size(12).into());
            }
        }
        if !results.albums.is_empty() {
            rows.push(text("\u{2014} Albums \u{2014}").size(12).into());
            for a in &results.albums {
                rows.push(text(format!("  {} \u{2014} {}", a.title, a.parent_title.as_deref().unwrap_or(""))).size(12).into());
            }
        }
        if !results.tracks.is_empty() {
            rows.push(text("\u{2014} Tracks \u{2014}").size(12).into());
            for t in &results.tracks {
                rows.push(text(format!("  {} \u{2014} {}", t.title, t.track_artist())).size(12).into());
            }
        }
    } else if !state.search.query.is_empty() {
        rows.push(text("(press Enter to execute search)").size(12).into());
    } else {
        rows.push(text("Type a query, then press Enter.").size(12).into());
    }
