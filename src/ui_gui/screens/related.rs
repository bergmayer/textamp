...
pub fn view(state: &AppState) -> Element<'_, GuiMessage> {
    ...
    for g in groups {
        rows.push(text(format!("\u{2014} {}", g.artist.title)).size(15).into());
        for a in &g.albums {
            rows.push(text(format!("  {}", a.title)).size(14).into());
        }
    }
    if rows.is_empty() {
        rows.push(text("No related artists loaded.").size(15).into());
    }
    ...
    container(column![text("Related artists").size(16), body].spacing(8).padding(12))
        .width(Length::Fill)
        .height(Length::Fill)
        .into()
}
