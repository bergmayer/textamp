    ...
    fn new(
        ...
        let y_scrollbar = if let Some(vertical) = show_scrollbar_y {
            ...
            let scroller = if ratio >= 1.0 {
                ...
                // (textamp local patch: 40px minimum so scrolling
                // 10k+-row libraries doesn't collapse the dragger to
                // a 2px sliver — the upstream default).
                let scroller_height =
                    (scrollbar_bounds.height * ratio).max(40.0);
                let scroller_offset =
        ...
        let x_scrollbar = if let Some(horizontal) = show_scrollbar_x {
            ...
            let scroller = if ratio >= 1.0 {
                ...
                // (textamp local patch — see vertical-scroller comment above).
                let scroller_length = (scrollbar_bounds.width * ratio).max(40.0);
                let scroller_offset =
