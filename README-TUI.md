...
A keyboard-driven Plex Music client for the terminal. Pairs with the
desktop GUI (`textamp-gui` — see [README-GUI.md](README-GUI.md)) — they
share the same shared back-end, on-disk caches, and feature set, so you
...
The top-level entry point is `build.sh` (Linux/macOS/WSL):
...
per-binary `build-tui.sh` script remains.
